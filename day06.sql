...
select ulke, count(*) as toplam_calisan from insan group by ulke;
...
select ulke, cinsiyet, count(isim),avg(yas) from insan group by ulke,cinsiyet;

-- SORU07: maas'ı 30000 den büyük olanların, 
-- ulke'lere göre ve cinsiyet'e göre çalışan sayısını, yas ortalamasını listeleyiniz.
select ulke,cinsiyet, count(isim) as calisan_sayisi, round(avg(yas),2) from insan 
where maas>3000 group by ulke,cinsiyet;

/*
HAVING: Sart belirtmek icin kullanilir
Group by komutundan sonra yer alir
where komutundan farki : Gruplmama yaptiktan sonra, cikan sonucda sart kosulacaksa
having komutu kullanilir.
*/
...
select ulke, round(avg(maas),2) as ortalama_maas from insan
